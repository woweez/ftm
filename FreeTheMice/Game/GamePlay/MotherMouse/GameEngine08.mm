...
GameEngine08Menu *layer08;

@implementation GameEngine08Menu
...
+(CCScene *) scene {
    ...
    layer08=[GameEngine08Menu node];
    [scene addChild:layer08 z:1];
    GameEngine08 *layer = [GameEngine08 node];
...
-(id) init
{
    if( (self=[super init])) {
        ...
        [layer08 addChild:progressBarBackSprite z:10];
        ...
        [layer08 addChild:cheeseCollectedSprite z:10];
        ...
        [layer08 addChild:progressBarSprite[0] z:10];
        ...
        for(int i=1;i<120;i++){
            ...
            [layer08 addChild:progressBarSprite[i] z:10];
        }
        ...
        [layer08 addChild:timeCheeseSprite z:10];
        ...
        [layer08 addChild:lifeMinutesAtlas z:10];
        ...
        [layer08 addChild:cheeseCollectedAtlas z:10];
        ...
        [layer08 addChild:clockBackgroundSprite z:0];
        ...
        [layer08 addChild:clockArrowSprite z:0];
        ...
        windowSprite.position=ccp(304,473);
        [self addChild:windowSprite z:0];
        
        windowSprite=[CCSprite spriteWithFile:@"window1.png"];
        windowSprite.position=ccp(414,473);
        [self addChild:windowSprite z:0];
        
        windowSprite=[CCSprite spriteWithFile:@"window1.png"];
        windowSprite.position=ccp(524,473);
        [self addChild:windowSprite z:0];
        ...
        pSprite.position=ccp(226,448);
        [self addChild:pSprite z:0];
        
        pSprite=[CCSprite spriteWithFile:@"move_platform2.png"];
        pSprite.position=ccp(616,479);
        [self addChild:pSprite z:0];
        
        pSprite=[CCSprite spriteWithFile:@"move_platform3.png"];
        pSprite.position=ccp(841,560);
        [self addChild:pSprite z:0];
        
        pSprite=[CCSprite spriteWithFile:@"move_platform2.png"];
        pSprite.position=ccp(1005,560);
        [self addChild:pSprite z:0];
        
        pSprite=[CCSprite spriteWithFile:@"move_platform3.png"];
        pSprite.position=ccp(834,392);
        [self addChild:pSprite z:0];
        
        pSprite=[CCSprite spriteWithFile:@"move_platform2.png"];
        pSprite.position=ccp(997,392);
        [self addChild:pSprite z:0];
        
        CCSprite *objectSprite=[CCSprite spriteWithFile:@"object3.png"];
        objectSprite.position=ccp(336,472);
        objectSprite.scale=0.8;
        [self addChild:objectSprite z:0];
        
        objectSprite=[CCSprite spriteWithFile:@"object3.png"];
        objectSprite.position=ccp(336,472);
        objectSprite.scale=0.8;
        [self addChild:objectSprite z:0];
        
        objectSprite=[CCSprite spriteWithFile:@"object6.png"];
        objectSprite.position=ccp(436,461);
        [self addChild:objectSprite z:0];
        
        objectSprite=[CCSprite spriteWithFile:@"object7.png"];
        objectSprite.position=ccp(438,498);
        [self addChild:objectSprite z:0];
        
        objectSprite=[CCSprite spriteWithFile:@"object9.png"];
        objectSprite.position=ccp(528,426);
        objectSprite.scale=0.6;
        [self addChild:objectSprite z:0];
        
        objectSprite=[CCSprite spriteWithFile:@"milk.png"];
        objectSprite.position=ccp(433,306);
        [self addChild:objectSprite z:0];
        
        objectSprite=[CCSprite spriteWithFile:@"knife2.png"];
        objectSprite.position=ccp(887,272);
        objectSprite.scale=0.5;
        [self addChild:objectSprite z:0];
        
        CCSprite *vegetablePlateSprite=[CCSprite spriteWithFile:@"vegetable_plate.png"];
        vegetablePlateSprite.position=ccp(610,285);
        [self addChild:vegetablePlateSprite z:1];
        
        vegetableCloseSprite=[CCSprite spriteWithFile:@"vegetable_close.png"];
        vegetableCloseSprite.position=ccp(493,285);
        vegetableCloseSprite.anchorPoint=ccp(0.0f, 0.3f);
        ...
        visibleSprite2=[CCSprite spriteWithFile:@"visible9.png"];
        visibleSprite2.position=ccp(468,473);
        ...
        [layer08 addChild:mouseTrappedBackground z:10];
        ...
        menu2.visible=NO;
        [layer08 addChild: menu2 z:10];
        
        CCSprite *holeSprite=[CCSprite spriteWithFile:@"hole.png"];
        holeSprite.position=ccp(960,435);
        ...
        dotSprite.position=ccp(390,255);
        dotSprite.scale=0.3;
...
-(void) addHudLayerToTheScene{
    ...
    hudLayer.tag = 8;
    [layer08 addChild: hudLayer z:2000];
...
-(void) addLevelCompleteLayerToTheScene{
    ...
    [layer08 addChild: lvlCompleteLayer z:2000];
}

-(void)initValue{
    //Cheese Count Importants
    DB *db = [DB new];
...
-(void) update: (ccTime) dt {
    ...
    if(visibleCount>=1){
        ...
        visibleSprite.opacity=250-visibleCount;
        visibleSprite2.opacity=250-visibleCount;
    }
...
-(void)level05{
    ...
    if(vegetableChe){
        ...
        gameFunc.vegetableCount-=1;
        gameFunc.vegetableCount=(gameFunc.vegetableCount<=0?0:gameFunc.vegetableCount);
    }
    
    vegetableCloseSprite.rotation=-(localVegetableCount/3.0)-9;
...
-(void)switchFunc{
    ...
    if(screenMoveChe){
        if(!vegetableChe){
            screenMoveCount+=1;
            if(screenMoveCount<90)
                screenShowY-=4;
            else if(screenMoveCount>100){
                screenShowY+=4;
                if(screenShowY>=screenShowY2){
                    screenShowY=screenShowY2;
                    gameFunc.switchCount=1;
                    screenMoveChe=NO;
                    if(gameFunc.screenMoveValue==0)
                        gameFunc.screenMoveValue=1;
                }
            }
            CGPoint copyHeroPosition = ccp(screenShowX, screenShowY);
            [self setViewpointCenter:copyHeroPosition];
        }else {
            screenMoveCount2+=0.5;
            if(screenMoveCount2<10)
                screenShowX+=1;
            else if(screenMoveCount2>10&&screenMoveCount2<40)
                screenShowY-=1;
            else if(screenMoveCount2>=70&&screenMoveCount2<90){
                screenShowY+=2;
                if(screenShowY>=screenShowY2){
                    screenShowY=screenShowY2;
                    screenMoveCount2=90;
                }
            }else if(screenMoveCount2>=90){
                screenShowX-=1;
...
//            NSLog(@"Object width and height==== %d %d %d", gameFunc.objectWidth,gameFunc.objectHeight,gameFunc.sideValueForObject);
            ...
            if(motherLevel == 8){
                if(heroSprite.position.x<750)
                    heroTrappedSprite.position = ccp(550, 310);
                else
                    heroTrappedSprite.position = ccp(900, 275);
...
-(void)heroLandingFunc{
    if(landingChe){
        ...
        if(heroJumpingAnimationCount>18){
            ...
            landingChe=NO;
            heroJumpingAnimationCount=0;
            if(!screenMoveChe&&!safetyJumpChe&&gameFunc.switchChe2&&gameFunc.screenMoveValue==0){
                screenMoveChe=YES;
                screenShowX=platformX;
                screenShowY=platformY;
...
- (void)ccTouchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if(!mouseWinChe&&!heroTrappedChe && !screenMoveChe&&!firstRunningChe){
...
- (void)ccTouchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if(!jumpingChe&&!runningChe&&heroJumpLocationChe&&!mouseWinChe&&motherLevel!=1&&!heroTrappedChe&& !screenMoveChe&&!firstRunningChe){
        activeVect = startVect - b2Vec2(location.x, location.y);
        jumpAngle = fabsf( CC_RADIANS_TO_DEGREES( atan2f(-activeVect.y, activeVect.x)));
        [self HeroLiningDraw:0];
    }
